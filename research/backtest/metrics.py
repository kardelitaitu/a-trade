...
def format_metrics_report(metrics: dict) -> str:
    """
    Format the metrics dict as a clean plain-text report.
    """
    lines = []
    sep = "-" * 60
    ...
    lines.append(sep)
    lines.append(f"  Initial equity:  ${capital:>10,.2f}")
    lines.append(f"  Final equity:    ${final:>10,.2f}")
    lines.append(f"  Total return:    {total_ret:>+10.2f}%")
    lines.append(f"  CAGR:            {cagr:>+10.2f}%")
    lines.append(f"  Period:          {metrics.get('total_years', 0):>8.2f} years")
    lines.append("")
    ...
    lines.append(f"  Longest DD:      {metrics.get('longest_dd_periods', 0):>7,} periods ({dd_days:.1f} days)")
    lines.append("")
    lines.append("Trade Statistics")
    lines.append(sep)
    lines.append(f"  Total trades:    {metrics.get('total_trades', 0):>10,}")
    lines.append(f"  Win rate:        {metrics.get('win_rate_pct', 0):>9.1f}%")
    lines.append(f"  Profit factor:   {metrics.get('profit_factor', 0):>10.2f}")
    lines.append(f"  Avg RR:          {metrics.get('avg_rr', 0):>9.2f}:1")
    lines.append(f"  Expectancy:      ${metrics.get('expectancy', 0):>+9.2f}")
    lines.append(f"  Avg win:         ${metrics.get('avg_win', 0):>+9.2f} ({metrics.get('avg_win_pct', 0):>+.2f}%)")
    lines.append(f"  Avg loss:        ${metrics.get('avg_loss', 0):>+9.2f} ({metrics.get('avg_loss_pct', 0):>+.2f}%)")
    lines.append(f"  Best trade:      ${metrics.get('best_trade_pnl', 0):>+9.2f}")
    lines.append(f"  Worst trade:     ${metrics.get('worst_trade_pnl', 0):>+9.2f}")
    lines.append(f"  Avg duration:    {metrics.get('avg_trade_duration_min', 0):>8.1f} min")
    lines.append(f"  Total fees:      ${metrics.get('total_fees', 0):>9,.2f}")
    lines.append("")
    return "\n".join(lines)
